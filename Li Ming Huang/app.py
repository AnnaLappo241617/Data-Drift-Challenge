...
# CONFIG
# =========================================================
st.set_page_config(page_title="Fraud Shield", layout="wide", page_icon="🛡️")
...
# Fonts always load regardless of theme
st.markdown("""
<style>
...
DARK = {
    ...
    "red_bg":       "#1a0808",
    "red_border":   "#3a1010",
    "card_grad":    "linear-gradient(135deg, #0f1829 0%, #0c1422 100%)",
    "chart_fig":    "#080c14",
    "chart_ax":     "#0a1020",
    "chart_edge":   "#1a2235",
    "chart_label":  "#5a6a8a",
...
LIGHT = {
    ...
    "red_bg":       "#fef2f2",
    "red_border":   "#fecaca",
    "card_grad":    "linear-gradient(135deg, #ffffff 0%, #f0f4ff 100%)",
    "chart_fig":    "#f4f6fb",
    "chart_ax":     "#ffffff",
    "chart_edge":   "#d0d8ea",
    "chart_label":  "#6b7a94",
...
[data-testid="metric-container"] {{
    background: {T['card_grad']} !important;
...
[data-testid="stDataFrame"] td {{
    ...
    color: {T['text']} !important;
    background-color: {T['bg4']} !important;
}}
.stButton > button {{
    background: linear-gradient(135deg, {T['accent']}, #1e40af) !important;
...
def apply_chart_theme():
    mpl.rcParams.update({
        "figure.facecolor":  T["chart_fig"],
        "axes.facecolor":    T["chart_ax"],
        "axes.edgecolor":    T["chart_edge"],
        "axes.labelcolor":   T["chart_label"],
        "axes.titlecolor":   T["chart_text"],
        ...
        "grid.linestyle":    "--",
        "grid.alpha":        0.6,
        "legend.facecolor":  T["chart_legend"],
        "legend.edgecolor":  T["chart_edge"],
        "legend.labelcolor": T["chart_text"],
...
def page_header(title: str, subtitle: str = ""):
    st.markdown(f"""
    <div style="padding:0.5rem 0 1.2rem; border-bottom:1px solid {T['border']}; margin-bottom:1.5rem;">
        <div style="font-family:'IBM Plex Mono',monospace; font-size:0.7rem; color:{T['accent3']};
        ...
        <div style="font-family:'IBM Plex Sans',sans-serif; font-size:1.55rem;
                    font-weight:300; color:{T['text2']}; letter-spacing:-0.01em;">
            {title}
        </div>
        {"<div style='font-size:0.85rem;color:" + T['text3'] + ";margin-top:4px;'>" + subtitle + "</div>" if subtitle else ""}
    </div>
    """, unsafe_allow_html=True)
...
def section_label(text: str):
    ...
    <div style="font-family:'IBM Plex Mono',monospace; font-size:0.68rem; font-weight:600;
                color:{T['accent3']}; letter-spacing:0.14em; text-transform:uppercase;
                margin:1.6rem 0 0.6rem; padding-bottom:6px; border-bottom:1px solid {T['border']};">
        {text}
    </div>
    """, unsafe_allow_html=True)
...
def status_banner():
    ...
    if not alerts:
        st.markdown(f"""
        <div style="display:flex;align-items:center;gap:12px;background:{T['green_bg']};
                    border:1px solid {T['green_border']};border-left:3px solid {T['green']};
                    border-radius:4px;padding:10px 16px;margin-bottom:1rem;">
            <span style="font-family:'IBM Plex Mono',monospace;font-size:0.72rem;
                         color:{T['green']};letter-spacing:0.1em;">● HEALTHY</span>
            <span style="font-family:'IBM Plex Sans',sans-serif;font-size:0.82rem;color:{T['text3']};">
                No alerts on <code style="background:none;color:{T['green']}">{batch}</code>
            </span>
        </div>
        """, unsafe_allow_html=True)
    else:
        is_crit = any("Critical" in a for a in alerts)
        colour = T["red"] if is_crit else T["amber"]
        bg     = T["red_bg"] if is_crit else T["amber_bg"]
        border = T["red_border"] if is_crit else T["amber_border"]
        ...
        """, unsafe_allow_html=True)
        for alert in alerts:
            c = T["red"] if "Critical" in alert else T["amber"]
            st.markdown(f"""
            <div style="font-family:'IBM Plex Mono',monospace;font-size:0.78rem;color:{c};
                        padding:3px 0 3px 16px;border-left:2px solid {c}44;margin-bottom:3px;">
                {alert}
            </div>
            """, unsafe_allow_html=True)
...
def empty_state(message: str):
    st.markdown(f"""
    <div style="background:{T['bg3']};border:1px dashed {T['border2']};border-radius:6px;
                padding:2rem;text-align:center;color:{T['text3']};
...
def info_block(html_content: str):
    st.markdown(f"""
    <div style="background:{T['bg2']};border:1px solid {T['border']};border-radius:6px;
                padding:1rem 1.2rem;">
        {html_content}
    </div>
    """, unsafe_allow_html=True)
...
def style_drift_df(df: pd.DataFrame):
    def row_style(row):
        if row.get("PSI Level") == "High":
            return [f"background-color:{T['red_bg']};color:{T['red']}"] * len(row)
        elif row.get("PSI Level") == "Moderate":
            return [f"background-color:{T['amber_bg']};color:{T['amber']}"] * len(row)
        elif row.get("High Risk") is True:
            return [f"background-color:{T['bg3']};color:{T['accent']}"] * len(row)
        return [""] * len(row)
...
def display_metric_with_delta(container, label, current_value, baseline_value):
    if pd.isna(current_value) or pd.isna(baseline_value):
        ...
        container.metric(label, f"{current_value:.3f}",
                         delta=f"{current_value - baseline_value:.3f}")
...
def detect_feature_drift(train_df, prod_df, feature_cols, alpha=0.05):
    ...
    return pd.DataFrame(rows).sort_values(
        ["Drift Detected", "PSI"], ascending=[False, False]).reset_index(drop=True)
...
def find_best_threshold(y_true, y_prob, metric="recall_with_precision", min_precision=0.10):
    best_t, best_s = DEFAULT_THRESHOLD, -1
...
def compute_baseline_metrics(base_df):
    ...
    bt, bts = find_best_threshold(y_te, y_prob, THRESHOLD_OPTIMIZATION_METRIC,
                                  MIN_PRECISION_FOR_THRESHOLD)
    metrics = evaluate_model(model, X_te, y_te, threshold=bt)
    metrics["Best Threshold"] = bt
    metrics["Best Threshold Score"] = bts
...
def degradation_check(current_metrics, baseline_metrics, drift_df, importance_df=None):
    ...
    mod_drift  = int((drift_df["PSI Level"] == "Moderate").sum())
    if high_drift >= 3:
        alerts.append(f"Critical: {high_drift} features show high drift")
    if mod_drift >= 5:
        alerts.append(f"Warning: {mod_drift} features show moderate drift")
    if importance_df is not None and not importance_df.empty:
        risk_df = cross_reference_drift_importance(drift_df, importance_df)
        hr = risk_df[risk_df["High Risk"]]["Feature"].tolist()
        if len(hr) >= 2:
            alerts.append(f"Critical: {len(hr)} high-importance features drifting: {', '.join(hr[:5])}")
        elif len(hr) == 1:
            alerts.append(f"Warning: High-importance feature drifting: {hr[0]}")
...
def register_new_model(new_model_version, model_name, trained_on,
                       activated_after_batch, validation_metrics):
...
def build_retraining_dataset(training_pool, batch_df):
    ...
    if len(training_pool) > MAX_HISTORICAL_RETRAIN_ROWS:
        ...
        historical_sample = pd.concat(
            [fh, nfh.sample(n=min(slots, len(nfh)), random_state=RANDOM_STATE)],
            ignore_index=True)
    else:
...
def retrain_and_promote_model(metadata, training_pool, batch_df, batch_name,
                               threshold, drift_df=None):
    ...
    register_new_model(metadata["current_model_version"], metadata["active_model_name"],
                       f"weighted_recent_retraining_through_{batch_name}",
                       batch_name, val_metrics)
    return metadata["current_model_version"], val_metrics

def build_monitor_log_row(batch_name, model_version_used, current_metrics,
                           baseline_metrics, drift_df, alerts, retrain_needed, new_model_version):
...
def process_batch(batch_file, threshold):
    ...
    append_monitor_log(build_monitor_log_row(
        batch_file, mv, current, baseline, drift_df, alerts, retrain, None))
...
def process_uploaded_batch(uploaded_file_obj, threshold):
    ...
    append_monitor_log(build_monitor_log_row(
        batch_name, mv, current, baseline, drift_df, alerts, retrain, None))
...
def ensure_state_initialized():
    ...
    pd.DataFrame([{
        "model_version": 1, "model_name": "Model v1", "trained_on": BASE_DATA_FILE,
        "activated_after_batch": "initial_training",
        "precision": baseline["Precision"], "recall": baseline["Recall"],
        "f1": baseline["F1"], "roc_auc": baseline["ROC-AUC"], "pr_auc": baseline["PR-AUC"],
    }]).to_csv(MODEL_REGISTRY_PATH, index=False)
    pd.DataFrame(columns=[
        "batch_name","model_version_used","precision","recall","f1","roc_auc","pr_auc",
        "delta_precision","delta_recall","delta_f1","delta_roc_auc","delta_pr_auc",
        "drifted_features","high_drift_features","top_drifted_features",
        "alerts","retrain_triggered","new_model_version",
...
# =========================================================
with st.sidebar:
    brand_col, toggle_col = st.columns([3, 1])
    with brand_col:
        st.markdown(f"""
        <div style="font-family:'IBM Plex Mono',monospace;font-size:0.75rem;
                    color:{T['accent']};letter-spacing:0.16em;text-transform:uppercase;
    ...
    with toggle_col:
        ...
        if st.button(T["toggle_icon"], help=T["toggle_label"], key="theme_toggle"):
    ...
    st.markdown(f"<hr style='border-color:{T['border']};margin:0.8rem 0;'>",
                unsafe_allow_html=True)
    st.markdown(f"""<p style='font-size:0.68rem;letter-spacing:0.1em;color:{T['accent3']};
                text-transform:uppercase;margin-bottom:0.4rem;'>Batch Controls</p>""",
                unsafe_allow_html=True)
    ...
    <div style="font-family:'IBM Plex Mono',monospace;font-size:0.68rem;
                color:{T['text3']};line-height:1.8;">
        Model · <span style="color:{T['accent']}">{metadata['active_model_name']}</span><br>
        Batches · <span style="color:{T['accent']}">{len(metadata.get('processed_batches',[]))}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    try:
        if source_mode == "Saved drift batch":
            if selected_batch == "No batches left":
                ...
                metrics, drift_df, alerts, retrain_needed, nmv, nmvm, scored = \
                    process_batch(selected_batch, threshold)
            ...
            if uploaded_file is None:
                ...
                bn, bdf, metrics, drift_df, alerts, retrain_needed, nmv, nmvm, scored = \
                    process_uploaded_batch(uploaded_file, threshold)
                st.session_state.update({
...
if page == "Overview":
    ...
    if "last_metrics" in st.session_state:
        ...
        empty_state("No batch processed yet — select a batch in the sidebar and click ▶ Process")

# =========================================================
# PAGE: METRICS
# =========================================================
elif page == "Metrics":
    page_header("Performance Metrics", "Batch evaluation against baseline · threshold trade-off")
    status_banner()
    ...
    if "last_metrics" not in st.session_state:
        ...
        if "last_drift_df" in st.session_state:
            ...
            rc = T["green"] if rd  >= 0 else T["red"]
            fc = T["green"] if fd  >= 0 else T["red"]
            pc = T["green"] if pd_ >= 0 else T["red"]
            info_block(f"""
            <div style="font-family:'IBM Plex Sans',sans-serif;font-size:0.88rem;
                        color:{T['text3']};line-height:1.8;">
                Drift detected in <span style="color:{T['text2']};font-weight:500">{drifted} features</span>
                including <span style="color:{T['red']}">{high} high-drift</span> features.<br>
        ...
        if "last_scored_batch_df" in st.session_state:
            ...
            for i, col in enumerate(["Precision", "Recall", "F1"]):
                ax.plot(tdf["Threshold"], tdf[col], color=CHART_COLORS[i], linewidth=2, label=col)
            ax.axvline(x=threshold, color=T["amber"], linestyle="--", linewidth=1, alpha=0.7)
            ax.text(threshold + 0.005, 0.05, f"current\n{threshold:.2f}",
                    color=T["amber"], fontsize=7, va="bottom")
...
# =========================================================
    ...
    if "last_drift_df" not in st.session_state:
        ...
        section_label("Feature Drift · Risk Table")
        st.caption("Red = high PSI · Yellow = moderate · Blue = high-risk (drifted + important)")
        ...
        if "last_batch_df" in st.session_state:
            ...
            ax.hist(training_pool[feature_option].dropna(), bins=50, alpha=0.55,
                    density=True, color=CHART_COLORS[0], label="Training Pool")
            ax.hist(st.session_state["last_batch_df"][feature_option].dropna(), bins=50,
                    alpha=0.55, density=True, color=CHART_COLORS[1], label="Incoming Batch")
        ...
        if "last_batch_df" in st.session_state:
            ...
            st.caption(f"Historical fraud rows: {len(train_fraud)} · "
                       f"Latest batch fraud rows: {len(batch_fraud)}")
            if len(batch_fraud) < 5:
                st.warning("Very few fraud cases — fraud-only drift results may be unstable.")
            else:
                fdrift = detect_feature_drift(
                    train_fraud[metadata["feature_cols"]],
                    batch_fraud[metadata["feature_cols"]],
                    metadata["feature_cols"], ALPHA)
        ...
        <div style="font-family:'IBM Plex Sans',sans-serif;font-size:0.85rem;
                    color:{T['text3']};line-height:1.8;">
            <span style="color:{T['accent']};font-weight:500;">Data drift</span> —
            input feature distributions changed. Detected via KS test and PSI.<br>
            <span style="color:{T['accent2']};font-weight:500;">Concept drift</span> —
            the relationship between features and fraud changed. A feature that once
            separated fraud may no longer do so, even if its distribution looks stable.
        </div>
        """)
...
# =========================================================
    ...
    if "last_scored_batch_df" not in st.session_state:
        ...
        detected = scored[scored["Fraud Detected"]==1].sort_values(
        ...
        f3c.metric("True Positives",
                   int(((scored["Fraud Detected"]==1) & (scored["Class"]==1)).sum()))
        ...
        colour_map = {
            "True Positive":  T["green"],
            "False Positive": T["amber"],
            "Missed Fraud":   T["red"],
            "True Negative":  T["accent"],
        }
        ...
        for i, label in enumerate(["True Positive","False Positive","Missed Fraud","True Negative"]):
            ...
            <div style="background:{T['bg2']};border:1px solid {T['border']};
                        border-left:3px solid {colour_map[label]};border-radius:4px;
                        padding:12px 16px;text-align:center;">
                <div style="font-family:'IBM Plex Mono',monospace;font-size:1.4rem;
                            font-weight:600;color:{colour_map[label]};">{count}</div>
                <div style="font-family:'IBM Plex Mono',monospace;font-size:0.68rem;
                            color:{T['text3']};letter-spacing:0.08em;margin-top:4px;">
                    {label.upper()}
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        display_cols = [c for c in ["Time","Hour","Amount","Fraud Probability",
                                     "Fraud Detected","Class","Detection Result"]
...
# =========================================================
    ...
    if "last_retrain_needed" not in st.session_state:
        ...
        st.markdown(f"""
        <div style="background:{T['amber_bg']};border:1px solid {T['amber_border']};
                    border-left:3px solid {T['amber']};border-radius:4px;
                    padding:12px 16px;font-family:'IBM Plex Sans',sans-serif;
                    font-size:0.88rem;color:{T['amber']};margin-bottom:1rem;">
        ...
        st.markdown(f"""
        <div style="background:{T['green_bg']};border:1px solid {T['green_border']};
                    border-left:3px solid {T['green']};border-radius:4px;
                    padding:12px 16px;font-family:'IBM Plex Sans',sans-serif;
                    font-size:0.88rem;color:{T['green']};">
...
# =========================================================
    ...
    if importance_df.empty:
        ...
        bars = ax.barh(top_imp["Feature"], top_imp["Importance"],
                       color=CHART_COLORS[0], alpha=0.85, height=0.65)
        for bar in bars:
...
# =========================================================
    ...
    if not monitor_log.empty:
        ...
        empty_state("No batches processed yet — timeline will appear here.")
    ...
    <div style="font-family:'IBM Plex Mono',monospace;font-size:0.78rem;
                color:{T['text3']};line-height:2;">
        monitoring_state/<br>
        ├── <span style="color:{T['accent']}">current_model.pkl</span> · active fraud detection model<br>
        ├── <span style="color:{T['accent']}">metadata.json</span> · model version + processed batches<br>
        ├── <span style="color:{T['accent']}">monitoring_log.csv</span> · full batch history<br>
        ├── <span style="color:{T['accent']}">model_registry.csv</span> · promoted model versions<br>
        ├── <span style="color:{T['accent']}">training_pool.csv</span> · accumulated training data<br>
        └── <span style="color:{T['accent']}">baseline_metrics.json</span> · benchmark reference
    </div>
    """)
